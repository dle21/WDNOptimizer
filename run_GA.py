...
class CustomGenerator(Generator):
    ...
    def generate(self, problem):
        if len(self.original) != len(problem.types):
...
def Min_Cost_GA(x):
    ...
    indexers = {"indexer_%d"%idx: val for idx, val in enumerate(x[:-1])}
    tank_indwexer[x[-1]]
...
problem = Problem(len(nwkp.pipes)+1+5, 2 , 0) # (Decisions variables, objectives, constraints)
problem.types[:-1] = Integer(0, len(available_diameters)-1)

# problem.types[-1] = Integer(0, 1)
...
plotting_results_export = {
    ...
    'manual_totex': manual_totex,
    'manual_penalties': manual_penalties,

    'existing_totex': existing_totex,
    'existing_penalties':existing_penalties,

    'xlim': [min(existing_totex, manual_totex, xlim_df.min()), max(existing_totex, manual_totex, xlim_df.max())],
    'ylim': [ylim_df.min(), ylim_df.max()],
}

pickle.dump(plotting_results_export, open('plotting_results.obj', 'wb'))
