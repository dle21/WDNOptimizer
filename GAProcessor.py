...
class GAProcessor:
    ...
    def get_initial_costs(self, min_p, max_hl):
        ...
        self.original = [np.where(available_diameters == round(self.nwk.wn.get_link(pipe).diameter * 1000, 0))[0][0] for pipe in self.nwk.pipes]
        # + \
    ...
    def run_GA(self, set_progress, gens, min_p, max_hl):
        ...
        self.problem.types[:len(self.nwk.pipes)] = Real(0, len(self.available_diameters)-1)
    ...
    def export_plot_results(self, output_obj):
        ...
        if self.proposed_inp_file is not None and self.proposed_inp_file != '':
            ...
            plotting_results_export['ylim'] = [min(self.manual_penalties, self.existing_penalties, min(pareto_h_penalties)) * 0.9, max(self.manual_penalties, self.existing_penalties, max(pareto_h_penalties)) * 1.1]
        ...
        for sol_idx in range(len(non_dominated_soln)):
            ...
            if not os.path.exists(output):
                ...
                solution = non_dominated_soln[sol_idx].variables

                diameters_result = [self.available_diameters[indexer] for indexer in
                [int(round(x, 0)) for x in solution[:len(self.nwk.pipes)]]]
                ...
                #     self.nwk.wn.get_link(pump).get_pump_curve().points = [(solution[len(self.nwk.pipes) + i], solution[len(self.nwk.pipes) + len(self.nwk.pumps) + i])]
    ...
    def Min_Cost_GA(self, x):
        ...
        if x == self.original and not self.nwk.original_results is None:
            ...
            for idx, pipe in enumerate(self.nwk.pipes):
                self.nwk.wn.get_link(pipe).diameter = self.available_diameters[int(round(x[idx], 0))]/1000
...
class CustomGenerator(Generator):
    def __init__(self, original, prop_random_p=0.2, prop_pop_random_p=0.2):
        super().__init__()
